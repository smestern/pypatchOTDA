...
from ot.bregman import sinkhorn, jcpot_barycenter
from ot.lp import emd
from ot.utils import unif, dist, kernel, cost_normalization, label_normalization, laplacian, dots
...
def laplacian(x):
    ...
    L = torch.diag(torch.sum(x, axis=0)) - x
    return L
...
def emd_laplace(a, b, xs, xt, M, sim='knn', sim_param=None, reg='pos', eta=1, alpha=.5,
    ...
    if sim == 'gauss':
        if sim_param is None:
            sim_param = 1 / (2 * (torch.mean(dist(xs, xs, 'sqeuclidean')) ** 2))
        ...
        with torch.no_grad():
            ...
            sS = torch.tensor(kneighbors_graph(X=xs.detach().numpy(), n_neighbors=int(sim_param)).toarray(), dtype=torch.float64)
            sS = (sS + sS.T) / 2
            sT = torch.tensor(kneighbors_graph(xt.detach().numpy(), n_neighbors=int(sim_param)).toarray(), dtype=torch.float64)
            sT = (sT + sT.T) / 2
    else:
    ...
    def f(G):
        return alpha * torch.trace(nx.dot(xt.T, nx.dot(G.T, nx.dot(lS, nx.dot(G, xt))))) \
            + (1 - alpha) * torch.trace(nx.dot(xs.T, nx.dot(G, nx.dot(lT, nx.dot(G.T, xs)))))
    ...
    def df(G):
        return alpha * nx.dot(ls2, nx.dot(G, xt2))\
            + (1 - alpha) * nx.dot(xs, nx.dot(xs.T, nx.dot(G, lt2)))
...
class BaseTransport(BaseEstimator):
    ...
    `transform` method should always get as inxut a `Xs` parameter

    `inverse_transform` method should always get as inxut a `Xt` parameter

    `transform_labels` method should always get as inxut a `ys` parameter

    `inverse_transform_labels` method should always get as inxut a `yt` parameter
    """
    ...
    def transform(self, Xs=None, ys=None, Xt=None, yt=None, batch_size=128):
        ...
        if check_params(Xs=Xs):
            ...
            if np.array_equal(self.xs_, Xs):
                ...
                transp = self.coupling_ / torch.sum(self.coupling_, 1)[:, None]

                # set nans to 0
                transp[~ torch.isfinite(transp)] = 0

                # compute transported samples
                transp_Xs = torch.matmul(transp, self.xt_)
            else:
